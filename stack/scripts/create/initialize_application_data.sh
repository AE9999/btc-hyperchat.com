...
(cd ../../application-configuration/db-only/scripts/ \
    && ./create_db_volume.sh "$CONFIGURATION") \
 && (cd ../../application-configuration/db-and-keycloak-only/scripts/ \
    && ./create_keycloak_setup.sh "$CONFIGURATION")
